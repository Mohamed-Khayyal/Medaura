...
export async function POST(
  ...
  try {
    ...
    const endpoint = role === "staff"
      ? `/api/payments/staff/bookings/${bookingId}/undo`
      : `/api/payments/doctor/bookings/${bookingId}/undo`;
    ...
    console.error(`Error undoing payment for booking ${bookingId}:`, error);
    return NextResponse.json(
      { success: false, error: error.message || "Failed to undo payment" },
      { status: 500 }
    );
  }
}
